...
test('contact service sends the form to the configured recipient', async () => {
  const data = { from_name: 'Test', phone: '+380955566767', question: 'Test', date: 'test', page_url: 'https://example.test' }
  let sent
  await sendContactRequest(data, async (url, options) => {
    sent = { url, options }
    return { ok: true, json: async () => ({ success: true }) }
  })
  assert.equal(sent.url, 'https://formsubmit.co/ajax/oleksandr.sviderskyi@nure.ua')
  assert.equal(sent.options.method, 'POST')
  const body = JSON.parse(sent.options.body)
  assert.equal(body["Ім'я"], data.from_name)
  assert.equal(body['Телефон'], data.phone)
  assert.equal(body['Питання'], data.question)
  assert.equal(body._subject, 'Нова заявка з сайту SUNITY')
